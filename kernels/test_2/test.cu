...
__global__ void what_is_my_id_2d_A(
    ...
    type * const block_y,
    type * const thread,
    type * const calc_thread,
    type * const x_thread,
    type * const y_thread,
    type * const grid_dimx,
    type * const block_dimx,
    type * const grid_dimy,
...
void test_gpu()
{
    ...
    for (uint8_t kernel = 0; kernel < 2; ++kernel) {
        ...
        //         std::cout << "CT: " << cpu_calc_thread[y][x] << " BKX: " << cpu_block_x[y][x] << " BKY: " << cpu_block_y[y][x] << " TID: " << cpu_thread[y][x] << " YTID: " << cpu_ythread[y][x] << " XTID: " << cpu_xthread[y][x] << " GDX: " << cpu_grid_dimx[y][x] << " BDX: " << cpu_block_x[y][x] << " GDY: " << cpu_grid_dimy[y][x] << " BDY: " << cpu_block_y[y][x] << std::endl;
        //     }
        // }
